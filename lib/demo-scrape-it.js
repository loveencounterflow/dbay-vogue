(function() {
  'use strict';
  var CHEERIO, CND, DBay, FS, GUY, H, HDML, Hnrss, PATH, SQL, Scraper, badge, debug, demo_1, demo_hnrss, demo_trends_as_table, demo_zvg24_net, demo_zvg_online_net, echo, got, help, info, relation_as_html, rpr, types, urge, warn, whisper;
...
order by sid, rank;`));
  ...
  demo_hnrss = async function() {
    ...
    H.tabulate("trends", hnrss.scr.db(SQL`select
    dsk                                           as dsk,
    sid                                           as sid,
    id                                            as id,
    rank                                          as rank,
  ...
  //-----------------------------------------------------------------------------------------------------------
  demo_trends_as_table = function(hnrss) {
    /* TAINT add view containing most recent session for each DSK */
    var d, dsk, dsk_html, id, id_html, rank, rank_html, ref, row, sid, sid_html, sql, tds/* TAINT use proper way to shorten string */, title_html, tr, trend, trend_html, ts, ts_html;
    sql = SQL`select * from scr_trends where sid = ( select max( sid ) from scr_trends );`;
    help('^5345^', HDML.open('table', {
      class: 'dbay-scr trends'
    }));
    ref = hnrss.scr.db(sql);
    for (row of ref) {
      ({dsk, sid, ts, id, rank, trend, d} = row);
      // debug '^4646^', row
      ...
      title_html = HDML.text(d.title.slice(0, 51));
      tds = [HDML.embrace('td', null, dsk_html), HDML.embrace('td', null, sid_html), HDML.embrace('td', null, id_html), HDML.embrace('td', null, ts_html), HDML.embrace('td', null, rank_html), HDML.embrace('td', null, trend_html), HDML.embrace('td', null, title_html)];
