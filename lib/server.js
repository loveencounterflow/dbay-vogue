(function() {
  'use strict';
  var CND, HTTP, Koa, PATH, Router, Socket_server, app, badge, debug, echo, file_server, file_server_cfg, help, http_server, info, io, mount, paths, router, rpr, server_cfg, urge, warn, whisper;
  ...
  app.use(async(ctx, next) => {
    ...
    ((base = ctx.state).greetings != null ? base.greetings : base.greetings = []).push("helo from X-Response-Time setter");
    return null;
  });
  ...
  router.get('home', '/', function(ctx) {
    ...
    help("^dbay-scr/server@7^", ctx.router.url('home', {
      query: {
        foo: 'bar'
      }
    }));
  ...
  app.use((ctx) => {
    ...
    ((base = ctx.state).greetings != null ? base.greetings : base.greetings = []).push("helo from content handler");
    return null;
  });
  ...
  //-----------------------------------------------------------------------------------------------------------
  http_server.listen(server_cfg, function() {
