(function() {
  ...
  Vogue_ops = class Vogue_ops {
    ...
    sparkline_from_trend(sparkline_data) {
      var plot_cfg;
      plot_cfg = {
        marks: [this._sparkline_get_line(sparkline_data), this._sparkline_get_dots(sparkline_data)],
        width: 500,
        height: 100,
        /* TAINT do not use magic numbers */
        x: {
    ...
    chart_from_trends(trends) {
      var i, len, marks, plot_cfg, sparkline_data;
      marks = [];
// trends.pid  = pid for sparkline_data in trends
      for (i = 0, len = trends.length; i < len; i++) {
        sparkline_data = trends[i];
        marks.push(this._XXX_sparkline_get_thin_line(sparkline_data));
        marks.push(this._XXX_sparkline_get_small_dots(sparkline_data));
      }
      plot_cfg = {
        marks: marks,
        width: 500,
