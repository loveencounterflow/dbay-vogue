...
insert into ${prefix}_posts ( sid, pid, rank, details )
  ...
  where true
    ...
    new_post(fields) {
      ...
      var details, dsk, html, ignore, pid, post, rank, session, sid, trend, ts;
      ...
      ({rank} = post);
      trend = JSON.parse(this.db.single_value(this.queries.trend_from_sid_pid, {sid, pid}));
      html = this._html_from_purpose('details', {dsk, sid, pid, ts, rank, trend, details});
      ignore = this.db.single_row(this.queries.insert_trends_html, {...post, html});
      return post;
    }
    ...
    get_latest_trends() {
      ...
      ({prefix} = this.cfg);
      R = [];
      ref1 = this.db(SQL`select trend from ${prefix}_latest_trends;`);
      for (row of ref1) {
