...
insert into ${prefix}_posts ( sid, pid, rank, details )
  ...
  from ${prefix}_trends where true
    ...
    _table_as_html(cfg) {
      ...
      var R, as_html, details, field, fields, format, i, inner_html, is_done, j, key, keys, len, len1, raw_value, ref1, ref2, ref3, ref4, ref5, ref6, row, row_nr, statement, table, table_i, title, use_fields, value;
      ({table, use_fields, fields} = cfg);
      keys = null;
      R = [];
      ...
      for (row of ref1) {
        ...
        if (row_nr === 1) {
          ...
          for (i = 0, len = keys.length; i < len; i++) {
            ...
            if ((field = fields[key]) != null) {
              ...
              title = (ref2 = field.title) != null ? ref2 : key;
            } else {
              title = key;
            }
            R.push(HDML.pair('th', {
        ...
        for (j = 0, len1 = keys.length; j < len1; j++) {
          ...
          raw_value = row[key];
          value = raw_value;
          field = (ref3 = fields[key]) != null ? ref3 : null;
          is_done = false;
          inner_html = null;
          if (field != null) {
            if (field.display === false) {
